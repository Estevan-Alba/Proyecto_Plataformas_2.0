...
class Activity_Menu_Estu : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener{
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_menu_estu)
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        when(item.itemId){
            R.id.nav_item1 -> {
                supportFragmentManager.commit {
                    replace<Fragment_Estudiante_Horario>(R.id.contenedorfracmentooes)
                    setReorderingAllowed(true)
                    addToBackStack("replacement")
                }
            }
            R.id.nav_item2 -> {
                supportFragmentManager.commit {
                    replace<Fragment_Estudiante_Materia>(R.id.contenedorfracmentooes)
                    setReorderingAllowed(true)
                    addToBackStack("replacement")
                }
            }
